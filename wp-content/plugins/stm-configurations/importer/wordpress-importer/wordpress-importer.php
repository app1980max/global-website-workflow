...
if ( class_exists( 'WP_Importer' ) ) {
	class WP_Import extends WP_Importer {
		 ...
		 */
		public function import( $file, $layout ) {
			$this->layout = $layout;
		...
		public function import_end() {
			...
			echo '<p>' . esc_html__( 'All done.', 'wordpress-importer' ) . ' <a href="' . esc_attr( admin_url() ) . '">' . esc_html__( 'Have fun!', 'wordpress-importer' ) . '</a></p>';
		...
		public function import_options() {
			$j = 0;
			?>
			<form action="<?php echo esc_attr( admin_url( 'admin.php?import=wordpress&amp;step=2' ) ); ?>" method="post">
		...
		public function process_terms() {
			...
			foreach ( $this->terms as $term ) {
				...
				if ( ! is_wp_error( $id ) ) {
					...
					/* translators: %1$s: term taxonomy, %2$s: term name. */
		...
		public function process_posts() {
			...
			foreach ( $this->posts as $post ) {
				...
				if ( ! post_type_exists( $post['post_type'] ) ) {
					printf(
						/* translators: %1$s: post title, %2$s: post name. */
				...
				if ( $post_exists && get_post_type( $post_exists ) === $post['post_type'] ) {
					/* translators: %1$s: singular post name, %2$s: post title. */
					printf( esc_html__( '%1$s &#8220;%2$s&#8221; already exists.', 'wordpress-importer' ), esc_html( $post_type_object->labels->singular_name ), esc_html( $post['post_title'] ) );
					echo '<br />';
					$post_id         = $post_exists;
					...
					if ( 'attachment' === $postdata['post_type'] ) {
						...
						if ( ! in_array( $this->layout, $upgraded_layouts, true ) ) {
							...
							if ( strpos( $image_name, 'unsplash_photo' ) !== false ) {
								...
								$remote_url = 'https://images.unsplash.com/' . $image_name . '?ixlib=rb-0.3.5&s=b447d7f07e2a211cd7f6745e77aeca03&auto=format&fit=crop&w=1300&h=800&q=10.jpeg';
							} else {
								$remote_url = 'http://crypterio.stylemixthemes.com/demoimages/' . $this->layout . '/' . $image_name;
							}
						}
					...
					foreach ( $post['terms'] as $term ) {
						...
						if ( ! $term_id ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								/* translators: %1$s: taxonomy, %2$s: term name. */
		...
		public function fetch_remote_file( $url, $post ) {
			...
			if ( '200' != $remote_response_code ) {
				...
				/* translators: %1$d: number, %2$s: number. */
		...
		public function backfill_parents() {
			...
			foreach ( $this->post_orphans as $child_id => $parent_id ) {
				...
				$local_child_id  = $local_parent_id;
