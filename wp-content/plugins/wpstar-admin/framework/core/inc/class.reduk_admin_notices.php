<?php
    ...
    /**
     * Reduk Framework Admin Notice Class
     * Makes instantiating a Reduk object an absolute piece of cake.
     *
    ...
    // Don't duplicate me!
    if ( ! class_exists( 'Reduk_Admin_Notices' ) ) {

        /**
         * Reduk API Class
         * Simple API for Reduk Framework
         *
         * @since       1.0.0
         */
        class Reduk_Admin_Notices {

            static public $_parent;

            public static function load() {
                add_action( 'wp_ajax_reduk_hide_admin_notice', array(
                    'Reduk_Admin_Notices',
            ...
            public static function adminNotices( $notices = array() ) {
                ...
                if ( ! empty( $notices ) ) {
                    ...
                    foreach ( $notices as $notice ) {
                        ...
                        $add_style = '';
                        if ( strpos( $notice['type'], 'reduk-message' ) != false ) {
                        ...
                        if ( true == $notice['dismiss'] ) {
                            ...
                            if ( ! get_user_meta( $userid, 'ignore_' . $notice['id'] ) ) {
                                ...
                                if ( $pagenow == 'admin.php' || $pagenow == 'themes.php' ) {
                                    ...
                                    // the reduk page slug if the GET is empty.
                                ...
                                if ( version_compare( $wp_version, '4.2', '>' ) ) {
                                    ...
                                    $css_class = esc_attr( $notice['type'] ) . ' reduk-notice notice is-dismissible reduk-notice';
                                    ...
                                    echo '<div ' . $add_style . ' class="' . esc_attr( $notice['type'] ) . ' notice is-dismissable"><p>' . wp_kses_post( $notice['msg'] ) . '&nbsp;&nbsp;<a href="?dismiss=true&amp;id=' . esc_attr( $notice['id'] ) . $pageName . $curTab . '">' . esc_html__( 'Dismiss', 'wpstr_framework' ) . '</a>.</p></div>';
                                }
                            }
                        } else {
                            // Standard notice
                        ...
                        <script>
                            jQuery( document ).ready(
                                function( $ ) {
                                    $( 'body' ).on(
                                        'click', '.reduk-notice.is-dismissible .notice-dismiss', function( event ) {
                                            var $data = $( this ).parent().find( '.dismiss_data' );
                                            $.post(
                                                ajaxurl, {
                                                    action: 'reduk_hide_admin_notice',
                                                    id: $data.attr( 'id' ),
                                                    nonce: $data.val()
                                                }
                                            );
