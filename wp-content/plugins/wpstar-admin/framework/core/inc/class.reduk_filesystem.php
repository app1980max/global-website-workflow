...
if (!class_exists('Reduk_Filesystem')) {
    class Reduk_Filesystem
    {
        /**
        ...
        public function __construct()
        {
            ...
            $this->parent->admin_notices[] = [
                ...
                'msg' =>
                    '<strong>' .
                    __('File Permission Issues', 'wpstr_framework') .
                    '</strong><br/>' .
                    sprintf(
                        __(
                            'We were unable to modify required files. Please check your permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%s" target="_blank">outlined here</a>.',
                            'wpstr_framework'
                        ),
        ...
        {
            if (
                isset($this->parent->ftp_form) &&
                !empty($this->parent->ftp_form)
            ) {
                ...
                echo '<strong>' .
                    ...
                    sprintf(
                        __(
                            'We were unable to modify required files. Please ensure that <code>%1s</code> has the proper read-write permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%2s" target="_blank">outlined here</a>.',
                            'wpstr_framework'
                        ),
                        Reduk_Helpers::cleanFilePath(
        ...
        public function execute($action, $file = '', $params = '')
        {
            ...
            if (!is_dir(RedukFramework::$_upload_dir)) {
                wp_mkdir_p(RedukFramework::$_upload_dir);
            }
            ...
            if ($this->parent->args['menu_type'] == 'submenu') {
                ...
                $base =
                    $page_parent . '?page=' . $this->parent->args['page_slug'];
            } else {
                $base = 'admin.php?page=' . $this->parent->args['page_slug'];
            }

            $url = wp_nonce_url($base, 'reduk-options');
        ...
        public function do_action($action, $file = '', $params = '')
        {
            ...
            if (
                !isset($params['chmod']) ||
                (isset($params['chmod']) && empty($params['chmod']))
            ) {
            ...
            if ($action == 'mkdir') {
                ...
                $action == 'copy' &&
                !isset($this->filesystem->killswitch)
            ) {
                if (
                    isset($this->parent->ftp_form) &&
                    !empty($this->parent->ftp_form)
            ...
            if (!$res) {
                ...
                //add_action( "reduk/page/{$this->parent->args['opt_name']}/form/before", array(
                //    $this,
                //    'ftp_form'
                //) );
            }
