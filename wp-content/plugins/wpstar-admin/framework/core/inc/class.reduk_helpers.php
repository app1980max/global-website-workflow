...
// Don't duplicate me!
    if ( ! class_exists( 'Reduk_Helpers' ) ) {

        /**
         * Reduk Helpers Class
         * Class of useful functions that can/should be shared among all Reduk files.
         *
         * @since       1.0.0
         */
        class Reduk_Helpers {
            ...
            public static function tabFromField( $parent, $field ) {
                foreach ( $parent->sections as $k => $section ) {
                    ...
                    if ( isset( $section['fields'] ) && ! empty( $section['fields'] ) ) {
                        if ( Reduk_Helpers::recursive_array_search( $field, $section['fields'] ) ) {
                            return $k;
                            continue;
                        }
                    }
...
//                }
            ...
            public static function trackingObject() {
                ...
                return Reduk_Helpers::getTrackingObject();
            }
            ...
            private static function redukAsPlugin() {
                return RedukFramework::$_as_plugin;
            }
            ...
            public static function localize( $localize ) {
                $reduk            = RedukFrameworkInstances::get_instance( $localize['args']['opt_name'] );
                $nonce            = wp_create_nonce( 'reduk-ads-nonce' );
                $base             = admin_url( 'admin-ajax.php' ) . '?action=reduk_p&nonce=' . $nonce . '&url=';
                $localize['rAds'] = Reduk_Helpers::rURL_fix( $base, $reduk->args['opt_name'] );
            ...
            public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
                ...
                $sysinfo['reduk_ver']      = esc_html( RedukFramework::$_version );
                $sysinfo['reduk_data_dir'] = RedukFramework::$_upload_dir;
                $f                         = 'fo' . 'pen';
                // Only is a file-write check
                $sysinfo['reduk_data_writeable'] = self::makeBoolStr( @$f( RedukFramework::$_upload_dir . 'test-log.log', 'a' ) );
                ...
                if ( ! class_exists( 'Browser' ) ) {
                    require_once RedukFramework::$_dir . 'inc/browser.php';
                }
                ...
                if ( $remote_checks == true ) {
                    $response = wp_remote_post( 'https://www.paypal.com/cgi-bin/webscr', array(
                        'sslverify'  => false,
                        'timeout'    => 60,
                        'user-agent' => 'RedukFramework/' . RedukFramework::$_version,
                        'body'       => array(
                    ...
                    $response = @wp_remote_get( 'http://redukframework.com/wp-admin/admin-ajax.php?action=get_reduk_extensions' );
                ...
                $sysinfo['reduk_instances'] = array();

                if ( ! empty( $reduk ) && is_array( $reduk ) ) {
                    foreach ( $reduk as $inst => $data ) {
                        Reduk::init( $inst );

                        $sysinfo['reduk_instances'][ $inst ]['args']     = $data->args;
                        $sysinfo['reduk_instances'][ $inst ]['sections'] = $data->sections;
                        foreach ( $sysinfo['reduk_instances'][ $inst ]['sections'] as $sKey => $section ) {
                            if ( isset( $section['fields'] ) && is_array( $section['fields'] ) ) {
                                foreach ( $section['fields'] as $fKey => $field ) {
                                    if ( isset( $field['validate_callback'] ) ) {
                                        unset( $sysinfo['reduk_instances'][ $inst ]['sections'][ $sKey ]['fields'][ $fKey ]['validate_callback'] );
                                    }
                                    if ( $field['type'] == "js_button" ) {
                                        if ( isset( $field['script'] ) && isset( $field['script']['ver'] ) ) {
                                            unset( $sysinfo['reduk_instances'][ $inst ]['sections'][ $sKey ]['fields'][ $fKey ]['script']['ver'] );
                                        }
                                    }
                        ...
                        $sysinfo['reduk_instances'][ $inst ]['extensions'] = Reduk::getExtensions( $inst );

                        if ( isset( $data->extensions['metaboxes'] ) ) {
                            $data->extensions['metaboxes']->init();
                            $sysinfo['reduk_instances'][ $inst ]['metaboxes'] = $data->extensions['metaboxes']->boxes;
                        }

                        if ( isset( $data->args['templates_path'] ) && $data->args['templates_path'] != '' ) {
                            $sysinfo['reduk_instances'][ $inst ]['templates'] = self::getRedukTemplates( $data->args['templates_path'] );
                        }
                    }
                }
            ...
            private static function getRedukTemplates( $custom_template_path ) {
                $filesystem         = Reduk_Filesystem::get_instance();
                $template_paths     = array( 'RedukFramework' => RedukFramework::$_dir . 'templates/panel' );
                $scanned_files      = array();
                $found_files        = array();
                ...
                foreach ( $scanned_files as $plugin_name => $files ) {
                    foreach ( $files as $file ) {
                        ...
                        if ( $theme_file ) {
                            $core_version  = self::get_template_version( RedukFramework::$_dir . 'templates/panel/' . $file );
                            ...
                            if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
                                ...
                                $found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'wpstr_framework' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
                            } else {
            ...
            public static function rURL_fix( $base, $opt_name ) {
                $url = $base . urlencode( 'http://ads.redukframework.com/api/index.php?js&g&1&v=2' ) . '&proxy=' . urlencode( $base ) . '';

                return Reduk_Functions::tru( $url, $opt_name );
            }
                    ...
                    foreach ( $files as $key => $value ) {
                        if ( ! in_array( $value, array( ".", ".." ) ) ) {
                            if ( is_dir( $template_path . DIRECTORY_SEPARATOR . $value ) ) {
                                $sub_files = reduk_scan_template_files( $template_path . DIRECTORY_SEPARATOR . $value );
            ...
            public static function get_extension_url( $dir ) {
                $ext_dir = Reduk_Helpers::get_extension_dir( $dir );
