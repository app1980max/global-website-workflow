...
// Don't duplicate me!
if ( !class_exists ( 'RedukFramework_slides' ) ) {

    /**
     * Main RedukFramework_slides class
     *
     * @since       1.0.0
     */
    class RedukFramework_slides {
        ...
        public function render () {
            ...
            $defaults = array(
                ...
                'content_title' => __ ( 'Slide', 'wpstr_framework' )
            );

            $this->field = wp_parse_args ( $this->field, $defaults );

            echo '<div class="reduk-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'wpstr_framework' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                    ...
                    echo '<div class="reduk-slides-accordion-group"><fieldset class="reduk-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="reduk-slides-header">' . $slide[ 'title' ] . '</span></h3><div>';
                    ...
                    echo '<img class="reduk-slides-image" id="image_image_id_' . $x . '" src="' . $slide[ 'thumb' ] . '" alt="" target="_blank" rel="external" />';
                    echo '</a>';
                    echo '</div>';

                    echo '<div class="reduk_slides_add_remove">';

                    echo '<span class="button media_upload_button" id="add_' . $x . '">' . __ ( 'Upload', 'wpstr_framework' ) . '</span>';
                    ...
                    echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . __ ( 'Remove', 'wpstr_framework' ) . '</span>';

                    echo '</div>' . "\n";

                    echo '<ul id="' . $this->field[ 'id' ] . '-ul" class="reduk-slides-list">';
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'title' ] ) : __ ( 'Title', 'wpstr_framework' );
                    ...
                    if ( $this->field[ 'show' ][ 'description' ] ) {
                        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'description' ] ) : __ ( 'Description', 'wpstr_framework' );
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'url' ] ) : __ ( 'URL', 'wpstr_framework' );
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion reduk-slides-remove">' . __ ( 'Delete', 'wpstr_framework' ) . '</a></li>';
            ...
            if ( $x == 0 ) {
                echo '<div class="reduk-slides-accordion-group"><fieldset class="reduk-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="reduk-slides-header">' . esc_attr ( sprintf ( __ ( 'New %s', 'wpstr_framework' ), $this->field[ 'content_title' ] ) ) . '</span></h3><div>';

                $hide = ' hide';

                echo '<div class="screenshot' . $hide . '">';
                echo '<a class="of-uploaded-image" href="">';
                echo '<img class="reduk-slides-image" id="image_image_id_' . $x . '" src="" alt="" target="_blank" rel="external" />';
                echo '</a>';
                echo '</div>';
                ...
                echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . __ ( 'Remove', 'wpstr_framework' ) . '</span>';
            ...
            echo '</div><a href="javascript:void(0);" class="button reduk-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add %s', 'wpstr_framework' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
        ...
        public function enqueue () {
            ...
            if ($this->parent->args['dev_mode']){
                ...
                wp_enqueue_style (
                    'reduk-field-slides-css', 
                    RedukFramework::$_url . 'inc/fields/slides/field_slides.css', 
                    array(),
                    time (), 
                    'all'
                );
            ...
            wp_enqueue_script (
                'reduk-field-slides-js', 
                RedukFramework::$_url . 'inc/fields/slides/field_slides' . Reduk_Functions::isMin () . '.js', 
                array( 'jquery', 'jquery-ui-core', 'jquery-ui-accordion', 'jquery-ui-sortable', 'reduk-field-media-js' ),
                time (), 
                true
            );
        }
