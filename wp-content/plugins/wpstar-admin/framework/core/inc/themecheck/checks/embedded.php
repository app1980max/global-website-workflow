            ...
            if ( $reduk ) {
                if ( ! isset( $_POST['reduk_wporg'] ) ) {
                    ...
                    if ( file_exists( $tracking ) ) {
                        $this->error[] = '<div class="reduk-error">' . sprintf( __('<span class="tc-lead tc-required">REQUIRED</span>: You MUST delete <strong> %s </strong>, or your theme will be rejected by WP.org theme submission because of Reduk.', 'wpstr_framework'), $tracking ) . '</div>';
                        $ret           = false;
                    }
                    ...
                    $args .= "<li><code>'database' => 'theme_mods'</code> (" . __( 'Optional', 'wpstr_framework' ) . ")</li>";
                    $args .= '</ol>';
                    $this->error[] = '<div class="reduk-error">' . __( '<span class="tc-lead tc-recommended">RECOMMENDED</span>: The following arguments MUST be used for WP.org submissions, or you will be rejected because of your Reduk configuration.', 'wpstr_framework' ) . $args . '</div>';
