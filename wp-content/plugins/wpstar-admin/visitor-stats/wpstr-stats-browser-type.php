...
<?php
    ...
    $entries = $wpdb->get_results( "SELECT  id,browser,knp_date FROM {$wpdb->prefix}wpstrwid WHERE knp_ts >= '" . $from_ts . "' AND  knp_ts <= '" . $to_ts . "' ORDER BY id DESC");
...
<?php if(sizeof($browser_stats) > 0){ ?>
      <div class="chartBox">
        <h4 class='widhead'><?php _e('Browsers used in last', 'wpstr_framework');  echo $days; _e('days', 'wpstr_framework'); ?></h4>
...
<?php if(sizeof($series_data) > 0){ ?>
      <div class="chartBox"><div class='widspacer10'></div>
          <h4 class='widhead'><?php _e('Browsers Used By Date', 'wpstr_framework'); ?></h4>
