...
if ($animation) {
	$css_class .= ' wpb_animate_when_almost_visible ' . $animation;
}
...
if( !empty($animation_delay) ){
	$animation_delay = 'style=animation-delay:' . $animation_delay . 's';
  }
?>
...
<?php if ($box_style == 'style_1') :
	...
	<?php if ($style == 'icon_left') { ?>
		<?php if (!empty($link['url'])): ?>
            <a href="<?php echo esc_url($link['url']) ?>" target="<?php echo esc_attr($link['target']); ?>" class="icon_box<?php echo esc_attr($css_class); ?> <?php echo esc_attr($style); ?> clearfix" <?php echo esc_attr($animation_delay); ?>>
		<?php else: ?>
