...
<?php if ( $events->have_posts() ): ?>
    <?php if( 'grid' === $event_style ) : ?>
        ...
        <?php if( $pagination_enable ) {
            ...
            crypterio_paging_nav( 'paging_view_posts-list', $events );
            echo '</div>'; }
        ?>
        ...
        <div class="stm_events_modern<?php echo esc_attr( $css_class ); ?>">
            ...
            <?php if ( $category != 'all' ) : ?>
                <?php if( $load_more_enable && $posts_per_page < $post_cat[0]->count ) : ?>
                    <div class="event_btn_loading vc_general vc_btn3 vc_btn3-size-lg vc_btn3-shape-rounded vc_btn3-style-outline vc_btn3-icon-left vc_btn3-color-theme_style_2"><i class="fas fa-refresh vc_btn3-icon" aria-hidden="true"></i> <?php esc_html_e( 'loading...', 'crypterio' ); ?></div>
                    <a href="#" data-page="1" data-load="<?php echo intval($posts_per_page); ?>" data-category="<?php echo esc_html($category); ?>" data-filter="<?php echo esc_html($events_filter); ?>" class="load_more_btn vc_general vc_btn3 vc_btn3-size-lg vc_btn3-shape-rounded vc_btn3-style-outline vc_btn3-icon-left vc_btn3-color-theme_style_2"><i class="fas fa-refresh vc_btn3-icon" aria-hidden="true"></i> <?php esc_html_e( 'load more', 'crypterio' ); ?></a>
                <?php endif; ?>
            <?php else: ?>
    ...
    <?php elseif( 'widget' === $event_style ) : ?>
        ...
        <?php wp_reset_postdata(); ?>
	<?php elseif( 'tiles' === $event_style ) : ?>
        <div class="stm_events_tiles<?php echo esc_attr( $css_class ); ?>">
		    <?php while ( $events->have_posts() ): $events->the_post(); ?>
			    <?php get_template_part('partials/content-event', 'tiles'); ?>
		    <?php endwhile; ?>
        </div>
		<?php wp_reset_postdata(); ?>
    <?php endif; ?>
