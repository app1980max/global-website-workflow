...
$demos     = array(
	'nft'           => array(
		'label' => esc_html__( 'NFT Landing', 'crypterio' ),
	),
	'ico_purple'    => array(
		'label' => esc_html__( 'ICO Landing (Isometric Purple)', 'crypterio' ),
	),
	'ico_isometric' => array(
		'label' => esc_html__( 'ICO Landing (Isometric Blue)', 'crypterio' ),
	),
	'ico_white'     => array(
		'label' => esc_html__( 'ICO Landing (Isometric White)', 'crypterio' ),
	),
	'ico'           => array(
		'label' => esc_html__( 'ICO Landing Page', 'crypterio' ),
	),
	'creative_ico'  => array(
		'label' => esc_html__( 'ICO Landing (Creative)', 'crypterio' ),
	),
	'ico_listing'   => array(
		'label' => esc_html__( 'ICO Listing', 'crypterio' ),
	),
	'crypto_blog'   => array(
		'label' => esc_html__( 'Cryptocurrency Blog', 'crypterio' ),
	),
	'token_sale'    => array(
		'label' => esc_html__( 'ICO Landing (Light)', 'crypterio' ),
	),
	'default'       => array(
		'label' => esc_html__( 'Blockchain Consulting', 'crypterio' ),
	),
	'ico_agency'    => array(
	...
	'advisor'       => array(
		'label' => esc_html__( 'ICO Advisor', 'crypterio' ),
	),
	'counselor'     => array(
		'label' => esc_html__( 'ICO Legal Advisor', 'crypterio' ),
	),
);
...
<div class="wrap about-wrap stm-admin-wrap  stm-admin-demos-screen">
	...
	if ( ! empty( $auth_code ) ) :
		...
		<div class="stm_demo_import_choices">
			...
			<?php foreach ( $demos as $demo_key => $demo_value ) : ?>
				<script type="text/javascript">
					stm_layouts['<?php echo esc_attr( $demo_key ); ?>'] = <?php echo wp_json_encode( crypterio_layout_plugins( $demo_key ) ); ?>;
				</script>
				<label class="
				<?php
				if ( $demo_key === $current_demo ) {
				...
				">
					<div class="inner">
						...
						<?php else : ?>
							<div class="install" data-layout="<?php echo esc_attr( $demo_key ); ?>"><?php esc_html_e( 'Import', 'crypterio' ); ?></div>
						<?php endif; ?>
		...
		<div class="stm_install__demo_popup">
			...
			<div class="inner">
				...
				<div class="privacy_policy">
					<h4><?php esc_html_e( 'Privacy Policy', 'crypterio' ); ?></h4>
					<div class="stm_install__demo_popup_close"></div>
					<div class="stm_plugins_status">
						<?php require_once 'privacy_policy.php'; ?>
					</div>
					<div class="privacy_policy_button decline" data-value="decline"><?php esc_html_e( 'Decline', 'crypterio' ); ?></div>
					<div class="privacy_policy_button" data-value="accept"><?php esc_html_e( 'Accept', 'crypterio' ); ?></div>
				...
				<div class="demo_install" style="display: none;">
					...
					<div class="stm_plugins_status">
						...
						foreach ( $plugins as $plugin ) :
							...
							$active_text = ( 'installed' === $active ) ? esc_html__( 'Installed & Activated', 'crypterio' ) : esc_html__( 'Not installed', 'crypterio' );
							?>
							<div id="<?php echo esc_attr( sanitize_text_field( 'stm_' . $plugin['slug'] ) ); ?>"
								class="stm_single_plugin_info <?php echo esc_attr( $active ); ?>"
								data-active="<?php echo esc_attr( $active ); ?>"
								data-slug="<?php echo esc_attr( sanitize_text_field( $plugin['slug'] ) ); ?>">
								<div class="image">
									<img src="<?php echo esc_url( get_template_directory_uri() . '/inc/admin/product_registration/assets/img/plugins/' . $plugin['slug'] . '.png' ); ?>"/>
								</div>
								<div class="title"><?php echo esc_html( sanitize_text_field( $plugin['name'] ) ); ?></div>
								<div class="status">
									<span><?php echo esc_html( sanitize_text_field( $active_text ) ); ?></span>
	...
	<?php else : ?>
		<div class="stm-admin-message">
			<?php esc_html_e( 'Please enter your Activation Token before running the Crypterio.', 'crypterio' ); ?>
		</div>
	<?php endif; ?>
...
<script type="text/javascript">
	(function ($) {
		var plugins = <?php echo wp_kses_post( html_entity_decode( wp_json_encode( wp_list_pluck( $plugins, 'slug' ) ) ) ); ?>;
		var layout = 'business';
		var plugin = false;
		var layout_plugins = [];
		...
		function performAjax(plugin_slug) {
			...
			$.ajax({
				...
				data: {
					...
					security: window.wp_data.crypterio_install_plugin
