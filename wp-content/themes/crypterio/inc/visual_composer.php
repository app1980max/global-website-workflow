	...
	if ( function_exists( 'vc_add_params' ) ) {
		...
		vc_add_params(
			...
			array(
				array(
					'type'       => 'checkbox',
					'heading'    => esc_html__( 'Show content in lightbox', 'crypterio' ),
		...
		if ( function_exists( 'vc_add_params' ) ) {
			...
			vc_add_params(
				...
				array(
					array(
						'type'       => 'checkbox',
						'heading'    => esc_html__( 'Particles background', 'crypterio' ),
			...
			vc_add_params(
				...
				array(
					...
					array(
						'type'             => 'colorpicker',
						'heading'          => esc_html__( 'Background On hover', 'crypterio' ),
						'param_name'       => 'custom_background_on_hover',
						'description'      => esc_html__( 'Select custom background color for your element on hover.', 'crypterio' ),
					...
					array(
						...
						'description'      => esc_html__( 'Select custom color for icon.', 'crypterio' ),
						'edit_field_class' => 'vc_col-sm-4',
						'dependency'       => array(
							'element' => 'style',
							'value'   => array( 'custom' ),
...
if ( ! function_exists( 'crypterio_vc_elements' ) ) {
	function crypterio_vc_elements() {
			...
			array(
				...
				'params'   => array(
					...
					array(
						...
						'heading'     => esc_html__( 'Animation Style', 'crypterio' ),
						'param_name'  => 'animation',
						'description' => esc_html__( 'Choose animation style', 'crypterio' ),
					),
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__( 'Animation delay', 'crypterio' ),
						'param_name'  => 'animation_delay',
						'description' => esc_html__( 'In seconds (example: 0.5)', 'crypterio' ),
						'admin_label' => false,
						'weight'      => 0,
					),
					array(
		...
		$vc_stat_counter = array(
			...
			array(
				'type'        => 'checkbox',
				'heading'     => esc_html__( 'Is on Screen', 'crypterio' ),
				'description' => esc_html__( 'When this option is enabled, counter starts immediately', 'crypterio' ),
				'param_name'  => 'is_on_screen',
				'value'       => array(
					esc_html__( 'Yes', 'crypterio' ) => 'yes',
				),
			),
			...
			array(
				...
				'params'   => array(
					...
					array(
						...
						'heading'     => esc_html__( 'Custom links', 'crypterio' ),
						'param_name'  => 'custom_links',
						'description' => esc_html__( 'Enter links for each slide (Note: divide links with linebreaks (Enter)).', 'crypterio' ),
					),
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__( 'Image size', 'crypterio' ),
					...
					array(
						...
						'params'     => array(
							array(
								...
								'heading'    => esc_html__( 'Time start', 'crypterio' ),
								'param_name' => 'timepicker_start',
								'holder'     => 'div',
							),
							array(
								'type'       => 'stm_timepicker_vc',
								'heading'    => esc_html__( 'Time end', 'crypterio' ),
							...
							array(
								'type'       => 'autocomplete',
								'heading'    => esc_html__( 'Select speakers', 'crypterio' ),
		...
		vc_map(
			array(
				'name'     => esc_html__( 'Countdown', 'crypterio' ),
				'base'     => 'stm_countdown',
				'icon'     => 'stm_countdown',
				'category' => esc_html__( 'STM', 'crypterio' ),
				'params'   => array(
					array(
						...
						'heading'    => esc_html__( 'Count to date', 'crypterio' ),
		...
		if ( defined( 'CALC_VERSION' ) ) {
			...
			$lists = array( esc_html__( 'Keep empty', 'crypterio' ) => 'select' );
			...
			if ( count( $data ) > 0 ) {
				...
				$lists = array( esc_html__( 'No calculator was found', 'crypterio' ) => 'select' );
			}

			vc_map(
				array(
					'name'     => esc_html__( 'Crypto Calculator', 'crypterio' ),
					'base'     => 'stm_crypto_calculator',
					'icon'     => 'stm_crypto_calculator',
					'category' => esc_html__( 'STM', 'crypterio' ),
					'params'   => array(
						array(
							'type'        => 'dropdown',
							'heading'     => esc_html__( 'Select calculator', 'crypterio' ),
							'param_name'  => 'calc_id',
							'value'       => $lists,
							'save_always' => true,
							'description' => '',
						...
						array(
							'type'       => 'dropdown',
							'heading'    => esc_html__( 'Calculator Style', 'crypterio' ),
							'param_name' => 'calc_style',
							'value'      => array(
								'Theme defaults' => 'theme_style',
		...
		$stm_works = array(
			...
			array(
				'type'        => 'autocomplete',
				'heading'     => esc_html__( 'Include Category', 'crypterio' ),
				'param_name'  => 'works_categories',
				'description' => esc_html__( 'Add Category. If not added show all category', 'crypterio' ),
				'settings'    => array(
					'multiple'       => true,
					'sortable'       => true,
					'min_length'     => 1,
		...
		vc_map(
			array(
				'name'     => esc_html__( 'Services With Tabs', 'crypterio' ),
				'base'     => 'stm_services_tabs',
				'category' => esc_html__( 'STM', 'crypterio' ),
				'params'   => array(
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__( 'Items Count', 'crypterio' ),
		...
		vc_map(
			array(
				...
				'params'      => array(
					array(
						'type'       => 'autocomplete',
						'heading'    => esc_html__( 'Check taxonomy to show posts', 'crypterio' ),
				...
				'params'   => array(
					array(
						...
						'value'      => array(
							...
							esc_html__( 'Bitcoin Price Widget', 'crypterio' ) => 'btcwdgt-price',
							esc_html__( 'Bitcoin News Widget', 'crypterio' ) => 'btcwdgt-news',
							esc_html__( 'Bitcoin Pool Widget', 'crypterio' ) => 'btcwdgt-pool',
							esc_html__( 'Bitcoin Forum Widget', 'crypterio' ) => 'btcwdgt-forum',
						),
						'std'        => 'btcwdgt-chart',
					),
					array(
	...
	function crypterio_add_fonts_pack( $option = 'stm_fonts' ) {
		...
		if ( ! empty( $custom_fonts ) ) {
			foreach ( $custom_fonts as $font_name => $custom_font ) {
				if ( 'stm_fonts_layout' === $option && empty( $custom_font['enabled'] ) ) {
					continue;
				}
...
vc_add_params(
	...
	array(
		array(
			'type'        => 'textfield',
			'heading'     => esc_html__( 'Background position', 'crypterio' ),
			'param_name'  => 'bg_pos',
			'description' => esc_html__( 'Enter background-position in CSS like format', 'crypterio' ),
		),
	)
);
...
vc_add_params(
	...
	array(
		...
		array(
			'type'        => 'colorpicker',
			'heading'     => esc_html__( 'Mobile background color', 'crypterio' ),
			'param_name'  => 'mobile_background',
			'description' => esc_html__( 'Enter background-position in CSS like format', 'crypterio' ),
		),
	)
);
$colors_dashed           = ( function_exists( 'vc_get_shared' ) ? vc_get_shared( 'colors-dashed' ) : array() );
$colors_animation_styles = ( function_exists( 'vc_get_shared' ) ? vc_get_shared( 'animation styles' ) : array() );
$vc_round_chart_params   = array(
	'params' => array(
		array(
			'type'        => 'textfield',
			'heading'     => esc_html__( 'Widget title', 'crypterio' ),
			'param_name'  => 'title',
			'description' => esc_html__( 'Enter text used as widget title (Note: located above content element).', 'crypterio' ),
			'admin_label' => true,
		),
		array(
			'type'        => 'dropdown',
			...
			'value'       => array(
				esc_html__( 'Pie', 'crypterio' )      => 'pie',
				esc_html__( 'Doughnut', 'crypterio' ) => 'doughnut',
			),
			'description' => esc_html__( 'Select type of chart.', 'crypterio' ),
			'admin_label' => true,
		),
		array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Style', 'crypterio' ),
			'description' => esc_html__( 'Select chart color style.', 'crypterio' ),
			'param_name'  => 'style',
			'value'       => array(
				esc_html__( 'Flat', 'crypterio' )   => 'flat',
				esc_html__( 'Modern', 'crypterio' ) => 'modern',
				esc_html__( 'Custom', 'crypterio' ) => 'custom',
			),
			'dependency'  => array(
		...
		array(
			...
			'description' => esc_html__( 'Select gap size.', 'crypterio' ),
			'std'         => 2,
		),
		array(
			'type'               => 'dropdown',
			'heading'            => esc_html__( 'Outline color', 'crypterio' ),
			'param_name'         => 'stroke_color',
			'value'              => $colors_dashed + array( esc_html__( 'Custom', 'crypterio' ) => 'custom' ),
			'description'        => esc_html__( 'Select outline color.', 'crypterio' ),
		...
		array(
			'type'        => 'colorpicker',
			'heading'     => esc_html__( 'Custom outline color', 'crypterio' ),
			'param_name'  => 'custom_stroke_color',
			'description' => esc_html__( 'Select custom outline color.', 'crypterio' ),
			'dependency'  => array(
		...
		array(
			'type'        => 'checkbox',
			'heading'     => esc_html__( 'Show hover values?', 'crypterio' ),
			'param_name'  => 'tooltips',
			'description' => esc_html__( 'If checked, chart will show values on hover.', 'crypterio' ),
			'value'       => array( esc_html__( 'Yes', 'crypterio' ) => 'yes' ),
			'std'         => 'yes',
		),
		array(
			...
			'params'     => array(
				array(
					...
					'heading'     => esc_html__( 'Title', 'crypterio' ),
					'param_name'  => 'title',
					'description' => esc_html__( 'Enter title for chart area.', 'crypterio' ),
					'admin_label' => true,
				),
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Description', 'crypterio' ),
					'param_name'  => 'stm_description',
					'description' => esc_html__( 'Enter description for chart area.', 'crypterio' ),
					'admin_label' => true,
				),
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Value', 'crypterio' ),
					'param_name'  => 'value',
					'description' => esc_html__( 'Enter value for area.', 'crypterio' ),
				),
				array(
					'type'               => 'dropdown',
					'heading'            => esc_html__( 'Color', 'crypterio' ),
					'param_name'         => 'color',
					'value'              => $colors_dashed,
					'description'        => esc_html__( 'Select area color.', 'crypterio' ),
					'param_holder_class' => 'vc_colored-dropdown',
				),
				array(
					'type'        => 'colorpicker',
					'heading'     => esc_html__( 'Custom color', 'crypterio' ),
					'param_name'  => 'custom_color',
					'description' => esc_html__( 'Select custom area color.', 'crypterio' ),
				),
			),
			'callbacks'  => array(
				'after_add' => 'vcChartParamAfterAddCallback',
		...
		array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Animation', 'crypterio' ),
			'description' => esc_html__( 'Select animation style.', 'crypterio' ),
			'param_name'  => 'animation',
			'value'       => $colors_animation_styles,
			'std'         => 'easeinOutCubic',
		),
		vc_map_add_css_animation(),
		array(
			'type'        => 'el_id',
			'heading'     => esc_html__( 'Element ID', 'crypterio' ),
			'param_name'  => 'el_id',
			// translators: %s: link
			'description' => sprintf( __( 'Enter element ID (Note: make sure it is unique and valid according to <a href="%s" target="_blank">w3c specification</a>).', 'crypterio' ), 'http://www.w3schools.com/tags/att_global_id.asp' ),
		),
		array(
			'type'        => 'textfield',
			'heading'     => esc_html__( 'Extra class name', 'crypterio' ),
		...
		array(
			'type'       => 'css_editor',
			'heading'    => esc_html__( 'CSS box', 'crypterio' ),
			'param_name' => 'css',
			'group'      => esc_html__( 'Design Options', 'crypterio' ),
		),
	),
...
vc_map_update( 'vc_round_chart', $vc_round_chart_params );
