...
if ( $sidebar_type == 'wp' ) {
	$sidebar_id = get_theme_mod( 'shop_wp_sidebar', 'crypterio-shop' );
} else {
...
$structure = crypterio_get_structure( $sidebar_id, $sidebar_type, get_theme_mod( 'shop_sidebar_position', 'right' ) ); ?>

<?php echo crypterio_sanitize_text_field($structure['content_before']); ?>
<?php if ( get_post_meta( get_the_ID(), 'disable_title', true ) ): ?>
	<h1 class="h2 page_title_2"><?php crypterio_page_title( true, esc_html__( 'News', 'crypterio' ), esc_html__( 'Careers', 'crypterio' ) ); ?></h1>
<?php endif; ?>
	<?php
		/**
