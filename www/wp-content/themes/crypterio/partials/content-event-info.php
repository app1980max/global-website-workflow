...
<h2><?php crypterio_page_title(); ?></h2>
...
<div class="event_info_table">
        ...
        <?php if( !empty( $stm_event_date_start ) || !empty( $stm_event_time_start ) || !empty( $stm_event_time_text ) ) : ?>
            <ul class="event-info__datetime" style="min-width: 134px;">
                ...
                <?php if( !empty( $stm_event_date ) ) : ?>
                    <li><i class="fas fa-calendar" aria-hidden="true"></i>  <?php echo esc_html( $stm_event_date ); ?></li>
                <?php endif; ?>
                <?php if( !empty( $stm_event_time_text ) ) : ?>
                    <li><i class="fas fa-clock"></i> <?php echo esc_html( $stm_event_time_text ); ?></li>
                <?php else: ?>
                    <?php if( !empty( $stm_event_time_start ) || !empty( $stm_event_time_end ) ) : ?>
                        <li><i class="fas fa-clock"></i> <?php echo (( $stm_event_time_end ) ? esc_html( $stm_event_time_start . ' ' . esc_html__('to', 'crypterio') . ' ' . $stm_event_time_end ) : esc_html( $stm_event_time_start ) ); ?></li>
                    <?php endif; ?>
                <?php endif; ?>
            </ul>
        <?php endif; ?>
    ...
    <div class="event_info">
        <ul>
            <?php if( $stm_event_venue = get_post_meta( get_the_ID(), 'stm_event_venue', true ) ) : ?>
                <li>
                    <i class="fas fa-map-marker" aria-hidden="true"></i> <?php echo esc_html( $stm_event_venue ); ?>
                </li>
            <?php endif; ?>
                ...
                <li>
                    <?php if( !empty( $stm_event_tel ) ) : $stm_event_tel = explode( ';', $stm_event_tel ); ?>
                        <?php foreach( $stm_event_tel as $stm_event_tel_val ) : ?>
                            <i class="fas fa-phone" aria-hidden="true"></i> <?php echo esc_attr( $stm_event_tel_val ); ?>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </li>
            <?php endif; ?>
    ...
    <div class="event_info text-right">
        <a href="#event-form-box" class="vc_general scroll_to_event_form vc_btn3 vc_btn3-size-lg vc_btn3-shape-rounded vc_btn3-style-flat vc_btn3-icon-right vc_btn3-color-theme_style_3" href="<?php the_permalink(); ?>">
            <?php esc_html_e( 'join now', 'crypterio' ); ?>
            <i class="vc_btn3-icon fas fa-chevron-right"></i>
        </a>
    </div>
    <?php endif; ?>
</div>