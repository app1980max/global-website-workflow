					...
					<?php if( !empty( $phones_arr ) && $phones_count > 0 ) : ?>
						<div class="text">
							<?php foreach( $phones_arr as $phone_number ) : $phones_showed++; ?>
								<a href="tel:<?php echo esc_attr( str_replace(' ', '', $phone_number) ); ?>"><?php sanitize_text_field( $phone_number ); ?></a><?php echo ( ( $phones_count > $phones_showed ) ? '<br>' : ''); ?>
							<?php endforeach; ?>
						</div>
					<?php endif; ?>
				</li>
	...
	<?php else: ?>
        ...
        <?php if( $facebook || $twitter || $linkedin || $google_plus || $skype ): ?>
            <ul class="socials">
                ...
                <?php endif; ?>
                <?php if( $linkedin ): ?>
                    <li><a href="<?php echo esc_url( $linkedin ); ?>" target="_blank" class="social-linkedin"><i class="fab fa-linkedin"></i></a></li>
                <?php endif; ?>
                <?php if( $google_plus ): ?>
                    <li><a href="<?php echo esc_url( $google_plus ); ?>" target="_blank" class="social-google-plus"><i class="fab fa-google-plus"></i></a></li>
                <?php endif; ?>
                <?php if( $skype ): ?>
                    <li><a href="skype:<?php echo esc_attr( $skype ); ?>" class="social-skype"><i class="fab fa-skype"></i></a></li>
                <?php endif; ?>
            </ul>
        <?php endif; ?>
