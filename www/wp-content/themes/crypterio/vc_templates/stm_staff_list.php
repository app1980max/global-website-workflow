                            ...
                            <?php if( has_post_thumbnail() ): ?>
                                <div class="staff_image">
                                    <a href="<?php the_permalink(); ?>">
                                        <?php the_post_thumbnail( 'crypterio-image-350x250-croped' ); ?>
                                    </a>
                                </div>
                            <?php endif; ?>
	...
	<?php else: ?>
		...
		<div class="staff_list<?php echo esc_attr( $css_class ); ?>">
			<ul>
				<?php while ( $testimonials->have_posts() ): $testimonials->the_post();
				if($style == 'team'):
				    get_template_part('partials/staff/team');
				elseif ($style == 'advisors'):
					get_template_part('partials/staff/advisors');
				else:
				?>
					<li>
						<?php if ( $style != 'grid' ) {
								...
								<div class="inner">
									...
									<div class="stm_invis">
										<div class="stm_excerpt">
											<?php if( $excerpt = crypterio_substr_text( get_the_excerpt(), $len ) ): ?>
												<p><?php echo esc_html( $excerpt ); ?></p>
											<?php endif; ?>
										</div>
										<a class="stm_link_bordered white" href="<?php the_permalink(); ?>"><?php esc_html_e('Read more', 'crypterio'); ?></a>
									</div>
								</div>
								<div class="staff_image">
									<a href="<?php the_permalink(); ?>">
						...
						<?php else: ?>
							...
							<div class="staff_info">
								...
								<?php endif; ?>
								<?php if ( $style != 'grid' ) : ?>
									<a href="<?php the_permalink(); ?>" class="button bordered size-sm icon_right"><?php esc_html_e( 'view profile', 'crypterio' ); ?> <i class="fas fa-chevron-right"></i></a>
								<?php else: ?>
									<a class="read_more" href="<?php the_permalink(); ?>">
										<span><?php esc_html_e( 'view profile', 'crypterio' ); ?></span>
										<i class=" fas fa-chevron-right stm_icon"></i>
									</a>
								<?php endif; ?>
							</div>
						<?php endif; ?>
...
<?php if( $style == 'carousel' ): ?>
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			...
			?>
			var arrows_en = <?php echo esc_js($arrows_en); ?>;
			...
			<?php echo esc_attr( $carousel_id ) ?>.slick({
				...
				prevArrow: "<div class=\"slick_prev\"><i class=\"fas fa-chevron-left\"></i></div>",
				nextArrow: "<div class=\"slick_next\"><i class=\"fas fa-chevron-right\"></i></div>",
				<?php endif; ?>
