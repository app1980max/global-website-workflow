...
$custom_icon_color = (!empty($atts['custom_icon_color'])) ? $atts['custom_icon_color'] : '';
$custom_background_on_hover = (!empty($atts['custom_background_on_hover'])) ? $atts['custom_background_on_hover'] : '';
...
$show_popup = (!empty($atts['stm_popup_iframe'])) ? true : false;
...
if(!empty($uniq)): ?>
    <style type="text/css">
        .<?php echo esc_attr($uniq); ?>:hover {
            background-color: <?php echo esc_attr($custom_background_on_hover); ?> !important;
        }
    </style>
<?php endif;
...
if ( 'true' === $add_icon ) {
	...
	if ( $icon_wrapper ) {
		$icon_html = '<i '.$icon_style.' class="vc_btn3-icon"><span class="vc_btn3-icon-inner ' . esc_attr( $icon_class ) . '"></span></i>';
	} else {
		$icon_html = '<i '.$icon_style.' class="vc_btn3-icon ' . esc_attr( $icon_class ) . '"></i>';
	}
...
if ( 'custom' === $style ) {
	...
	$gradient_css[] = 'background-image: -webkit-linear-gradient(left, ' . $gradient_color_1 . ' 0%, ' . $gradient_color_2 . ' 50%,' . $gradient_color_1 . ' 100%) !important';
	$gradient_css[] = 'background-image: linear-gradient(to right, ' . $gradient_color_1 . ' 0%, ' . $gradient_color_2 . ' 50%,' . $gradient_color_1 . ' 100%) !important';
...
if( !empty($animation_delay) ){
	$wrapper_attributes[] = ' style="animation-delay:' . $animation_delay . 's" ';
}
