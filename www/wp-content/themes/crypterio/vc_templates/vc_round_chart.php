...
foreach ( $values as $k => $v ) {
	...
	$data[]          = array(
		...
		'stm_description' => isset( $v['stm_description'] ) ? $v['stm_description'] : '',
	);
}

$options[]        = 'data-vc-type="' . esc_attr( $type ) . '"';
$round_chart_data = array(
	'datasets' => array(
		array(
			'data'            => $datasetValues,
...
if ( $legend ) {
	foreach ( $data as $v ) {
		...
		$legend_html .= '<li><span style="background-color:' . $color . '"></span>' . $v['label'] . '<p>' . $v['stm_description'] . '</p></li>';
	}
...
echo wp_kses( $output, crypterio_allowed_html() );
