	...
	">
		<ul class="news_list posts_per_row_<?php echo esc_attr( $posts_per_row ); ?>">
			...
			while ( $query->have_posts() ) :
				...
				if ( 'ico' === $config['layout'] || 'nft' === $config['layout'] ) :
					get_template_part( 'partials/news/ico' );
				elseif ( 'creative_ico' === $config['layout'] ) :
						get_template_part( 'partials/news/ico' );
				elseif ( 'ico_white' === $config['layout'] || 'ico_purple' === $config['layout'] ) :
					get_template_part( 'partials/news/ico' );
				elseif ( 'ico_isometric' === $config['layout'] ) :
					get_template_part( 'partials/news/ico_isometric' );
				else :
					...
					$post_thumbnail = $post_thumbnail ? $post_thumbnail['thumbnail'] : '';
					?>
					<li>
							...
							<?php if ( has_post_thumbnail() && ! $disable_preview_image ) : ?>
								<div class="image">
									<a href="<?php the_permalink(); ?>">
										<?php echo wp_kses_post( html_entity_decode( $post_thumbnail ) ); ?>
									</a>
								</div>
							<?php endif; ?>
