...
{
	return 'https://support.stylemixthemes.com/';
}

function crypterio_theme_docs_url()
{
...
{
    check_ajax_referer('crypterio_install_plugin', 'security');
	$r = array();
...
function crypterio_check_plugin_active($slug) {
	/*if just slug*/
	if(strpos($slug, '.php') === false) $slug = crypterio_get_plugin_main_path($slug);
