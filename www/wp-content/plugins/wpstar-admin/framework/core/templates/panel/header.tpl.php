<?php
    ...
    $tip_title = __( 'Developer Mode Enabled', 'wpstr_framework' );
    ...
    if ( $this->parent->dev_mode_forced ) {
        ...
        if ( Reduk_Helpers::isWpDebug() ) {
            $is_debug  = true;
            $debug_bit = __( 'WP_DEBUG is enabled', 'wpstr_framework' );
        }

        $localhost_bit = '';
        if ( Reduk_Helpers::isLocalHost() ) {
            $is_localhost  = true;
            $localhost_bit = __( 'you are working in a localhost environment', 'wpstr_framework' );
        }

        $conjunction_bit = '';
        if ( $is_localhost && $is_debug ) {
            $conjunction_bit = ' ' . __( 'and', 'wpstr_framework' ) . ' ';
        }

        $tip_msg = __( 'This has been automatically enabled because', 'wpstr_framework' ) . ' ' . $debug_bit . $conjunction_bit . $localhost_bit . '.';
    } else {
        $tip_msg = __( 'If you are not a developer, your theme/plugin author shipped with developer mode enabled. Contact them directly to fix it.', 'wpstr_framework' );
    }
        ...
        <div class="display_header">
            ...
            <?php if ( isset( $this->parent->args['dev_mode'] ) && $this->parent->args['dev_mode'] ) { ?>
                <?php /* ?><div clas   s="reduk-dev-mode-notice-container reduk-dev-qtip"
                    ...
                    <span
                        class="reduk-dev-mode-notice"><?php _e( 'Developer Mode Enabled', 'wpstr_framework' ); ?></span>
                </div><?php */ ?>
            <?php } elseif (isset($this->parent->args['forced_dev_mode_off']) && $this->parent->args['forced_dev_mode_off'] == true ) {/* ?>
                ...
                <div class="reduk-dev-mode-notice-container reduk-dev-qtip" 
                     ...
                     qtip-content="<?php echo esc_attr( $tip_msg ); ?>">
                    <span
                        class="reduk-dev-mode-notice" style="background-color: #FF001D;"><?php _e( 'FORCED DEV MODE OFF ENABLED', 'wpstr_framework' ); ?></span>
                </div>
